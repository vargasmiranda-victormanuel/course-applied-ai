...
# Cargar .env a nivel de módulo para que la API key esté disponible
# ANTES de que create_agent inicialice el cliente de OpenAI
load_dotenv()

# ── API moderna de LangGraph V1.x ─────────────────────────────────────────────
# En LangGraph V1.0, create_react_agent se movió a langchain.agents como create_agent
...
@tool
def calculadora(numero1: float, numero2: float, operacion: str) -> str:
    """Realiza cálculos matemáticos entre dos números.

    Operaciones soportadas (escríbelas en español o inglés):
    - suma / add: numero1 + numero2
    - resta / subtract: numero1 - numero2
    - multiplicacion / multiply: numero1 * numero2
    - division / divide: numero1 / numero2
    - potencia / power: numero1 ** numero2
    - raiz / root: numero1 ** (1 / numero2)  (raíz n-ésima de numero1)
    - modulo / mod: numero1 % numero2

    Retorna el resultado de la operación como texto.
    """
    print(f"🧮 Calculadora: {numero1} [{operacion}] {numero2}")
    ...
    if op in ("suma", "add", "+"):
        ...
        resultado = numero1 - numero2
    elif op in ("multiplicacion", "multiplicación", "multiply", "*", "x"):
        resultado = numero1 * numero2
    elif op in ("division", "división", "divide", "/"):
        if numero2 == 0:
            return "❌ Error: división por cero no está permitida."
        resultado = numero1 / numero2
    elif op in ("potencia", "power", "**", "^"):
        resultado = numero1 ** numero2
    elif op in ("raiz", "raíz", "root"):
        if numero2 == 0:
            return "❌ Error: el índice de la raíz no puede ser cero."
        resultado = numero1 ** (1 / numero2)
    elif op in ("modulo", "módulo", "mod", "%"):
        if numero2 == 0:
            return "❌ Error: módulo por cero no está permitido."
        resultado = numero1 % numero2
    else:
        return (
            f"❌ Operación '{operacion}' no reconocida. "
            "Usa: suma, resta, multiplicacion, division, potencia, raiz, modulo."
        )

    print(f"   ✅ Resultado: {resultado}")
    return f"🧮 Resultado de {numero1} {operacion} {numero2} = **{resultado}**"


@tool
def translator(texto: str, idioma: str) -> str:
    """Traduce un texto al idioma destino indicado.

    Útil cuando el usuario quiere traducir fragmentos de texto, frases, párrafos
    o palabras a cualquier idioma. El idioma debe indicarse en español o inglés
    (por ejemplo: 'inglés', 'francés', 'alemán', 'japanese', 'portuguese').
    Llama al LLM para realizar la traducción con precisión y naturalidad.
    """
    print(f"🌐 Traduciendo texto al idioma: '{idioma}'...")

    # Llamada directa al LLM para obtener la traducción
    llm = ChatOpenAI(model="gpt-4o", temperature=0)
    prompt_traduccion = (
        f"Traduce el siguiente texto al {idioma}. "
        "Responde ÚNICAMENTE con el texto traducido, sin explicaciones adicionales.\n\n"
        f"Texto original:\n{texto}"
    )
    respuesta = llm.invoke(prompt_traduccion)
    traduccion = respuesta.content

    print(f"   ✅ Traducción completada ({len(traduccion)} caracteres)")
    return f"🌐 **Traducción al {idioma}:**\n\n{traduccion}"


@tool
def resumen_texto(texto: str) -> str:
    """Recibe un texto extenso y genera un resumen conciso y claro.

    Útil cuando el usuario proporciona un artículo, documento, párrafo largo
    o cualquier bloque de texto que necesita ser condensado manteniendo
    las ideas principales. El resumen se genera en el mismo idioma del texto.
    """
    print("📄 Generando resumen del texto...")

    # Llamada directa al LLM para resumir
    llm = ChatOpenAI(model="gpt-4o", temperature=0)
    prompt_resumen = (
        "Genera un resumen conciso y claro del siguiente texto. "
        "Conserva las ideas principales y elimina los detalles secundarios. "
...
SYSTEM_PROMPT = """Eres un asistente inteligente y versátil con acceso a múltiples herramientas especializadas.
Analiza cada solicitud del usuario y selecciona la herramienta más adecuada. Puedes encadenar
varias herramientas en secuencia cuando la tarea lo requiera.

## Herramientas disponibles y cuándo usarlas:

- `buscar_web`: Para investigar temas de IA, tendencias tecnológicas y frameworks.
- `guardar_nota`: Para persistir reportes, resúmenes o hallazgos importantes en disco.
- `fecha_actual`: Para obtener la fecha y hora actuales del sistema.
- `calculadora`: Para cualquier cálculo matemático (suma, resta, multiplicación, división,
  potencia, raíz, módulo). Llámala con dos números y el nombre de la operación.
- `translator`: Para traducir un texto a otro idioma. Requiere el texto y el idioma destino.
- `resumen_texto`: Para resumir un texto extenso en un extracto conciso.

## Flujo de trabajo:

1. **Interpreta** la solicitud del usuario y determina qué herramienta(s) son necesarias.
2. **Ejecuta** las herramientas en el orden correcto. Si una herramienta proporciona datos
   que otra necesita, encadénalas (el resultado de una alimenta la siguiente).
3. **Itera** tantas veces como sea necesario hasta tener una respuesta completa.
4. **Responde** con un resumen claro al usuario.
...
agent = create_agent(
    model="openai:gpt-4o",
    tools=[buscar_web, guardar_nota, fecha_actual, calculadora, translator, resumen_texto],
...
if __name__ == "__main__":
    ...
    # (load_dotenv ya fue llamado a nivel de módulo)
    ...
    print("=" * 62)
    print("🤖  AGENTE MULTIHERRAMIENTA  —  LangChain + LangGraph")
    print("=" * 62)
    print("⚙️  Agente inicializado con modelo: gpt-4o")
    print("🛠️  Herramientas disponibles:")
    print("     🔍 buscar_web    📝 guardar_nota   📅 fecha_actual")
    print("     🧮 calculadora   🌐 translator     📄 resumen_texto")
    print("─" * 62)
    print("💡 Ejemplos de uso:")
    print("   • 'Investiga las tendencias en agentes de IA para 2026'")
    print("   • 'Cuánto es 2 elevado a la potencia 10'")
    print("   • Traduce 'Hello world' al japonés")
    print("   • 'Resume este texto: [pega tu texto aquí]'")
    print("─" * 62)

    # El usuario ingresa su propio prompt en cada ejecución
    pregunta = input("\n❓ ¿Qué deseas hacer? > ").strip()

    if not pregunta:
        print("⚠️  No ingresaste ninguna pregunta. Saliendo...")
        raise SystemExit(0)
